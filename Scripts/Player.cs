...
public partial class Player : CharacterBody3D
{
	...
	public override void _Ready()
	{
		...
		if (!InputMap.HasAction("jump"))
		{
			...
			var jumpEvent = new InputEventKey();
			jumpEvent.Keycode = Key.Space;
			InputMap.ActionAddEvent("jump", jumpEvent);
		}
		
		if (!InputMap.HasAction("attack"))
		{
			InputMap.AddAction("attack");
			var attackEvent = new InputEventKey();
			attackEvent.Keycode = Key.Z;
			InputMap.ActionAddEvent("attack", attackEvent);
		}
