...
本作ではSkul: The Hero Slayerのような「シナジーを構築する楽しさ」を重視するため、ラン中の成長要素として以下の3軸を中心に構成する。

#### 🎲 1. ランダムスキルピック（選択式）
- ボス撃破・祠・ランダムイベント等で出現
- プレイヤーは提示された3候補から1つを取得
- 選択肢にはタグ（例：闇／分身／召喚）付きで、ビルド軸が可視化される

#### 🧪 2. スキル変異ルーン（Rune/Modifier）
- 既存スキルに対し「属性／形状／発動条件」などを変更するルーンを装着可能
- ルーンはスロット制限あり（最大2つなど）
- 例：影分身に「毒属性」＋「敵撃破時CT短縮」のルーンを付加

#### 📿 3. 聖遺物（Relics）
- 特殊効果を持つパッシブ装備。最大数に制限あり（例：5個まで）
- 一部は「タグシナジー効果」を持ち、ビルドの完成度が高まる
- 例：「闇＋分身」タグを3つ以上持つと与ダメ＋20%

### 4.4 シナジー設計指針（ビルド多様性）

本作では以下のようなタグベースのシナジー構築により、毎回異なるプレイスタイルを楽しめるよう設計する。
...
- **シナジー例**
    - 「闇」タグを持つスキルが3つ以上 → 暗闇状態の敵への与ダメ上昇
    - 「分身」＋「DoT」タグを併せ持つ → 分身攻撃で毒を付与
...
| バージョン | 更新日     | 変更内容                                                                   |
| ---------- | ---------- | -------------------------------------------------------------------------- |
| 0.2.1      | 2024-03-21 | ドキュメント管理ルールに準拠した更新<br>- メタデータの更新<br>- 日付の修正 |
| 0.2.0      | 2024-03-21 | プロジェクト計画の詳細化                                                   |
